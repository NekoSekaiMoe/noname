...
game.import("character", function () {
	return {
		...
		character: {
			...
			jsrg_zhangju: ["male", "qun", 4, ["jsrgqiluan", "jsrgxiangjia"]],
			//兴
			jsrg_jiananfeng: ["female", "jin", 3, ["jsrgshanzheng", "jsrgxiongbao", "jsrgliedu"]],
			jsrg_wenyang: ["male", "wei", 4, ["jsrgfuzhen"]],
			jsrg_zhugedan: ["male", "wei", 4, ["jsrgzuozhan", "jsrgcuibing", "jsrglangan"]],
			jsrg_wangjun: ["male", "jin", 4, ["jsrgchengliu", "jsrgjianlou"]],
			jsrg_limi: ["male", "shu", 3, ["jsrgciyin", "jsrgchendu"], ["border:jin"]],
			jsrg_simazhao: ["male", "wei", 4, ["jsrgqiantun", "jsrgxiezheng", "jsrgzhaoxiong"]],
			jin_jsrg_simazhao: ["male", "jin", 4, ["jsrgweisi", "jsrgxiezheng", "jsrgdangyi"], ["zhu", "unseen"]],
			jsrg_dengai: ["male", "wei", 4, ["jsrgpiqi", "jsrgzhoulin"]],
			jsrg_simaliang: ["male", "jin", "3/4", ["jsrgsheju", "jsrgzuwang"]],
			jsrg_tufashujineng: ["male", "qun", 4, ["jsrgqinrao", "jsrgfuran"]],
			jsrg_lukang: ["male", "wu", 4, ["jsrgzhuwei", "jsrgkuangjian"], ["clan:吴郡陆氏"]],
			jsrg_malong: ["male", "jin", 4, ["jsrgfennan", "jsrgxunji"]],
		},
		characterSort: {
			jsrg: {
				jiangshanrugu_qi: [
		...
		skill: {
			...
			jsrgchushi: {
				...
				ai: {
					order(item, player) {
						...
						// 临时修改（by 棘手怀念摧毁）
						const history = target.getHistory("gain", evt => evt.getParent("jsrgwentian", true) && evt.getParent("jsrgwentian", true).player == player);
			...
			jsrgzhaobing: {
				...
				content: function () {
					...
					var prompt2 =
						"弃置所有手牌，令至多" +
						get.cnNumber(num) +
						"名其他角色依次选择一项：1.正面向上交给你一张【杀】；2.失去1点体力";
					player
				...
				content: function () {
					...
					target
						.chooseCard(
							...
							"交给其一张牌并令其此回合使用【杀】的次数上限+1；或点击“取消”令其视为对你使用一张【杀】",
							"he"
						)
						.set("ai", (card) => {
			...
			jsrgxiongbao: {
				...
				async cost(event, trigger, player) {
					event.result = await player
						.chooseCard(get.prompt("jsrgxiongbao"), 2, "本次议事你展示两张手牌")
						.forResult();
				},
				async content(event, trigger, player) {
				...
				subSkill: {
					effect: {
						...
						filter(event, player) {
							...
							// 临时修改（by 棘手怀念摧毁）
							return event.card && event.card.name == "juedou" && event.getParent("jsrgbeizhi", true);
				...
				async cost(event, trigger, player) {
					const cards = trigger.getd().filter(card => {
						...
						return game.hasPlayer(current => {
							...
							// 临时修改（by 棘手怀念摧毁）
							if(evt && evt.es && evt.es.includes(card)) return true;
							if(evt && evt.js && evt.js.includes(card)) return true;
				...
				async content(event, trigger, player) {
					...
					if (numx > num) {
						...
						while(game.hasPlayer(current => current.countCards("ej")) && i < discard) {
							...
							if (result.bool) {
								const result2 = await player
									.discardPlayerCard(result.targets[0], "ej", [1, discard - i])
									.forResult();
								if (result2.bool) {
			...
			jsrgzhoulin: {
				...
				direct: true,
				async content(event, trigger, player) {
					if (event.triggername == "phaseBegin") player.markAuto(event.name, game.filterPlayer(current => !player.inRange(current) && current != player));
					...
					if (!evt.jsrgfennan?.includes(target)) result = { index: 1 };
						...
						result = await target
							...
							.set("ai", () => {
								...
								// 临时修改（by 棘手怀念摧毁）
								return Math.random() > player.countCards("e") / player.countEnabledSlot() ? 0 : 1;
		...
		translate: {
			...
			jsrgchaozheng: "朝争",
			jsrgchaozheng_info:
				"准备阶段，你可以令所有其他角色议事。若结果为：红色，意见为红色的角色各回复1点体力；黑色，意见为红色的角色各失去1点体力。然后若所有意见均相同，你摸X张牌（X为此次议事的角色数）。",
			jsrgshenchong: "甚宠",
			jsrgshenchong_info:
			...
			jsrgzhaobing: "诏兵",
			jsrgzhaobing_info:
				"结束阶段，你可以弃置所有手牌，然后令至多X名其他角色依次选择一项：1.正面向上交给你一张【杀】；2.失去1点体力（X为你本次弃置的牌数）。",
			jsrgzhuhuan: "诛宦",
			jsrgzhuhuan_info:
				"准备阶段，你可以展示所有手牌并弃置所有【杀】，然后令一名其他角色选择一项：1.弃置等量的牌，然后受到1点伤害；2.令你摸等量的牌，然后你回复1点体力。",
			jsrgyanhuo: "延祸",
			jsrgyanhuo_info:
			...
			jsrgpingtao: "平讨",
			jsrgpingtao_info:
				"出牌阶段限一次。你可以令一名其他角色选择一项：1.交给你一张牌，然后你于此回合使用【杀】的次数上限+1；2.令你视为对其使用一张【杀】。",
			jsrgjuelie: "绝烈",
			jsrgjuelie_info:
			...
			jsrgguanhuo: "观火",
			jsrgguanhuo_info:
				"①出牌阶段，你可以视为使用一张【火攻】。②当你因〖观火①〗使用的【火攻】结算结束后，若此牌未造成过伤害，且：若{你此阶段发动〖观火①〗的次数为1，则你于此阶段造成渠道为【火攻】的伤害时，此伤害+1}，否则你失去〖观火〗。",
			jsrgjuxia: "居下",
			jsrgjuxia_info:
			...
			jsrgyingmen: "盈门",
			jsrgyingmen_info:
				"锁定技。①游戏开始时，你将武将牌堆中随机四张武将牌置于你的武将牌上，称为“访客”。②回合开始时，若你的“访客”数小于4，你随机从武将牌堆中将“访客”补至四张。",
			jsrgpingjian: "评鉴",
			jsrgpingjian_info:
			...
			jsrgjuezhi: "绝质",
			jsrgjuezhi_info:
				"①当你失去一张装备区里的装备牌后，你可以废除对应的装备栏。②你的回合每阶段限一次。当你使用牌对目标角色造成伤害时，你令此伤害+X（X为其装备区里的牌与你已废除的装备栏中相同副类别的数量）。",
			jsrgjizhao: "急召",
			jsrgjizhao_info:
				"准备阶段或结束阶段，你可以令一名角色选择一项：1.使用一张手牌；2.令你可以移动其区域里的一张牌。",
			jsrg_yangbiao: "起杨彪",
			jsrg_yangbiao_prefix: "起",
			jsrgzhaohan: "昭汉",
			jsrgzhaohan_info:
				"锁定技。准备阶段，若本局游戏：未洗过牌，你回复1点体力；洗过牌，你失去1点体力。",
			jsrgrangjie: "让节",
			jsrgrangjie_info:
				"当你受到1点伤害后，你可以移动场上的一张牌，然后你可以于弃牌堆中选择获得一张本回合进入弃牌堆且与此牌花色相同的牌。",
			jsrgyizheng: "义争",
			jsrgyizheng_info:
			...
			jsrgjuxiang: "拒降",
			jsrgjuxiang_info:
				"当你不于摸牌阶段得到牌后，你可以弃置之，令当前回合角色于此回合额定的出牌阶段内使用【杀】的次数上限+X（X为你以此法弃置的牌的花色数）。",
			jsrg_liubei: "起刘备",
			jsrg_liubei_prefix: "起",
			jsrgjishan: "积善",
			jsrgjishan_info:
				"①每回合限一次。当一名角色受到伤害时，你可以失去1点体力并防止此伤害，然后你与其各摸一张牌。②每回合限一次。当你造成伤害后，你可以令一名体力值最小且你对其发动过〖积善①〗的角色回复1点体力。",
			jsrgzhenqiao: "振鞘",
			jsrgzhenqiao_info:
				"锁定技。①你的攻击范围+1。②当你使用【杀】指定目标后，若你的武器栏为空且未废除，你令此【杀】的效果额外结算一次。",
			jsrg_wangyun: "起王允",
			jsrg_wangyun_prefix: "起",
			jsrgshelun: "赦论",
			jsrgshelun_info:
				"出牌阶段限一次。你可以选择一名你攻击范围内的角色，然后令除其外所有手牌数不大于你的角色议事。若结果为：红色，你弃置其一张牌；黑色，你对其造成1点伤害。",
			jsrgfayi: "伐异",
			...
			jsrgtushe: "图射",
			jsrgtushe_info:
				"当你使用非装备牌指定目标后，你可以展示所有手牌（无牌则不展示）。若你没有基本牌，你可以摸X张牌（X为此牌指定的目标数）。",
			jsrgtongjue: "通绝",
			jsrgtongjue_info:
			...
			jsrgpingrong: "平戎",
			jsrgpingrong_info:
				"每轮限一次。一名角色的回合结束时，你可以移去一名角色的“猎”，然后你于此回合后执行一个额外回合。该回合结束后，若你于此回合未造成过伤害，你失去1点体力。",
			...
			jsrglinghua: "灵化",
			jsrglinghua_info:
				"①准备阶段，你可以执行目标角色为你的【闪电】效果。若你未因此受到伤害，你可以令一名角色回复1点体力。②结束阶段，你可以执行目标角色为你且判定效果反转的【闪电】效果。若你未因此受到伤害，你可以对一名角色造成1点雷电伤害。",
			sbyingmen: "盈门",
			sbyingmen_info:
				"锁定技。①游戏开始时，你将武将牌堆中随机四张武将牌置于你的武将牌上，称为“访客”。②回合开始时，若你的“访客”数小于4，你随机从武将牌堆中将“访客”补至四张。",
			sbpingjian: "评鉴",
			...
			jsrglipan: "离叛",
			jsrglipan_info:
				"回合结束时，你可以变更势力，然后摸X张牌并执行一个额外的出牌阶段。此阶段结束时，所有与你势力相同的角色依次可以将一张牌当【决斗】对你使用（X为与你势力相同的其他角色数）。",
			jsrgqingxi: "轻袭",
			jsrgqingxi_info:
			...
			jsrgwuchang: "无常",
			jsrgwuchang_info:
				"锁定技。①当你获得其他角色的牌后，你变更势力为与其相同。②当你使用【杀】或【决斗】对与你势力相同的目标角色造成伤害时，此伤害+1，然后变更势力为群。",
			jsrgqingjiao: "轻狡",
			jsrgqingjiao_info:
			...
			jsrgguyin: "孤吟",
			jsrgguyin_info:
				"准备阶段，你可以翻面，且令所有其他男性角色依次选择是否翻面。然后你和所有背面朝上的角色轮流各摸一张牌，直到你们累计以此法得到X张牌（X为场上存活角色与死亡角色中男性角色数）。",
			jsrgzhangdeng: "帐灯",
			jsrgzhangdeng_info:
			...
			jsrgbiaozhao: "表召",
			jsrgbiaozhao_info:
				"准备阶段，你可以选择两名其他角色A和B。直到你的下回合开始时或你死亡后，A对B使用牌无次数和距离限制，且B对你使用的牌造成的伤害+1。",
			jsrgyechou: "业仇",
			jsrgyechou_info:
			...
			jsrgcangchu: "仓储",
			jsrgcangchu_info:
				"一名角色的结束阶段，你可以令至多X名角色各摸一张牌，若X大于存活角色数，则改为各摸两张牌（X为你于此回合得到的牌数）。",
			jsrgshishou: "失守",
			jsrgshishou_info:
			...
			jsrgzhenfeng: "针锋",
			jsrgzhenfeng_info:
				"出牌阶段每种类别各限一次。你可以视为使用一张存活角色的技能描述中包含的基本牌或普通锦囊牌（无距离和次数限制）。当此牌对技能描述中包含此牌的角色生效时，你对其造成1点伤害。",
			jsrg_zhangfei: "转张飞",
			...
			jsrgshacheng: "沙城",
			jsrgshacheng_info:
				"①游戏开始时，你将牌堆顶的两张牌置于武将牌上，称为“城”。②当一名角色使用【杀】结算结束后，你可以移去一张“城”，令此牌的其中一名目标角色摸X张牌（X为该角色本回合失去过的牌数且至多为5）。",
			jsrgninghan: "凝寒",
			jsrgninghan_info:
			...
			jsrgfuni: "伏匿",
			jsrgfuni_info:
				"锁定技。①你的攻击范围终值为0。②一轮游戏开始时，你令任意名角色获得共计X张【影】（X为存活角色数的一半，向上取整）。③当有牌进入弃牌堆后，若其中有【影】，你于本回合使用牌无距离限制且不能被响应。",
			jsrgchuanxin: "穿心",
			jsrgchuanxin_info:
				"一名角色的结束阶段，你可以将一张牌当【杀】使用。当一名角色受到渠道为此【杀】的伤害时，此伤害+Y（Y为其本回合回复过的体力值）。",
			jsrg_huangzhong: "转黄忠",
			jsrg_huangzhong_prefix: "转",
			jsrgcuifeng: "摧锋",
			jsrgcuifeng_info:
				"限定技。出牌阶段，你可以视为使用一张单目标的伤害类牌（无距离限制）。此回合结束时，若此牌未造成伤害或造成的伤害值大于1，你重置〖摧锋〗。",
			jsrgdengnan: "登难",
			jsrgdengnan_info:
			...
			jsrgfenjian: "奋剑",
			jsrgfenjian_info:
				"每回合各限一次。当你需要对其他角色使用【决斗】或【桃】时，你可以令你本回合受到的伤害+1，视为使用之。",
			...
			jsrgjiaohao: "骄豪",
			jsrgjiaohao_info:
				"①其他角色的出牌阶段限一次。其可以将手牌中的一张装备牌置于你的装备区。②准备阶段，你获得X张【影】（X为你空置装备栏数的一半，向上取整）。",
			jsrg_pangtong: "转庞统",
			...
			jsrgyangming: "养名",
			jsrgyangming_info:
				"出牌阶段限一次。你可以与一名角色拼点，若其：没赢，你可以与其重复此流程；赢，其摸X张牌，然后你回复1点体力（X为其此阶段没赢的次数）。",
			jsrg_hansui: "转韩遂",
			...
			jsrgfushan: "负山",
			jsrgfushan_info:
				"出牌阶段开始时，所有其他角色可以依次交给你一张牌并令你此阶段使用【杀】的次数上限+1。此阶段结束时，若你使用【杀】的次数未达到上限且此阶段以此法交给你牌的角色均存活，你失去2点体力，否则你将手牌摸至体力上限。",
			//江山如故·合
			...
			jsrgchushi: "出师",
			jsrgchushi_info:
				"出牌阶段限一次。若你不为主公，你可以与主公议事。若结果为：红色，你与其各摸一张牌，若你与其手牌数之和小于7，重复此流程；黑色，当你于本轮内造成属性伤害时，此伤害+1。",
			jsrgyinlve: "隐略",
			jsrgyinlve_info:
			...
			jsrgzhendan: "镇胆",
			jsrgzhendan_info:
				"①你可以将一张非基本手牌当任意基本牌使用或打出。②当你受到伤害后，你摸X张牌并令该技能本轮失效（X为本轮所有角色执行过的回合数且至多为5）。③一轮游戏开始时，若上一轮该技能未因〖镇胆②〗失效过，你摸Y张牌（Y为上一轮所有角色执行过的回合数且至多为5）。",
			jsrg_simayi: "合司马懿",
			...
			jsrgfusha: "伏杀",
			jsrgfusha_info:
				"限定技。出牌阶段，若你的攻击范围内仅有一名角色，你可以对其造成X点伤害（X为你的攻击范围，至多为游戏人数）。",
			...
			jsrgfangjie: "芳洁",
			jsrgfangjie_info:
				"准备阶段，若你没有蓄谋牌，你回复1点体力并摸一张牌，否则你可以弃置你区域里的任意张蓄谋牌并失去〖芳洁〗。",
			jsrg_caofang: "合曹芳",
			...
			jsrgdanxin: "丹心",
			jsrgdanxin_info:
				"你可以将一张牌当【推心置腹】使用，你展示以此法交出与得到的牌，以此法得到♥牌的角色回复1点体力，然后你至目标角色的距离+1直到回合结束。",
			jsrgfengxiang: "封乡",
			jsrgfengxiang_info:
			...
			jsrgfuhai: "浮海",
			jsrgfuhai_info:
				"出牌阶段限一次。你可以令所有有手牌的其他角色同时展示一张手牌，然后你选择一个方向并摸X张牌（X为该方向上的角色展示的点数连续严格递增或严格递减的牌数，至少为1）。",
			jsrg_zhangxuan: "合张嫙",
			...
			jsrgzunwei: "尊位",
			jsrgzunwei_info:
				"出牌阶段限一次。你可以选择一名其他角色并选择执行一项，然后移除该选项：1.将手牌数摸至与该角色相同（最多摸五张）；2.将其装备牌移至你的装备区，直到你装备区的牌数不少于其；3.将体力值回复至与该角色相同。",
